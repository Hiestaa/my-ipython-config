...
# Lines to execute upon start-up
packagesToImport = [
    'sys',
    'os',
    're',
...
c.TerminalIPythonApp.exec_lines += ['import %s' % p for p in packagesToImport]
c.TerminalIPythonApp.exec_lines += [
    'from %s import %s' % (package[0], ', '.join(package[1:]))
    for package in modulesToImport
]

c.TerminalIPythonApp.exec_lines.append(
    """
def initial_imports():
    global path, relativedelta, requests, dateutil
    ...
    print('')
    """ % (', '.join(p for p in packagesToImport),
           ', '.join(', '.join(modulesToImport[x][1:]) for x in xrange(len(modulesToImport)))))
c.TerminalIPythonApp.exec_lines.append("initial_imports()")
